...
import com.appspa.update.proxy.impl.AbstractUpdateParser;
import com.appspa.update.utils.UpdateUtils;
import com.appspa.demo.entity.ApiResult;
import com.appspa.demo.entity.AppVersionInfo;
import com.appspa.demo.http.XHttpUpdateHttpService;
...
public class XUpdateServiceParser extends AbstractUpdateParser {
    ...
    public UpdateEntity parseJson(String json) throws Exception {
        ...
        if (apiResult != null) {
            ...
            if (appVersionInfo.getUpdateStatus() == DefaultUpdateParser.APIConstant.NO_NEW_VERSION) {
                ...
                mDownloadEntity.setDownloadUrl(getDownLoadUrl(appVersionInfo.getDownloadUrl()));
                mDownloadEntity.setSize(appVersionInfo.getApkSize());
                mDownloadEntity.setMd5(appVersionInfo.getApkMd5());
                mDownloadEntity.setWholeMd5(appVersionInfo.getApkMd5());
                updateEntity.setHasUpdate(true)
                        //兼容一下
                        .setUpdateContent(appVersionInfo.getModifyContent().replaceAll("\\\\r\\\\n", "\r\n"))
                        .setVersionCode(appVersionInfo.getVersionCode())
                        .setDownLoadEntity(mDownloadEntity)
    ...
    private AppVersionInfo doLocalCompare(AppVersionInfo appVersionInfo) {
        ...
        if (appVersionInfo.getUpdateStatus() != DefaultUpdateParser.APIConstant.NO_NEW_VERSION) {
            ...
            //最新版本小于等于现在的版本，不需要更新
            if (lastVersionCode <= UpdateUtils.getVersionCode(AppSpace.getContext())) {
