...
import static com.appspace.update.entity.UpdateError.ERROR.INSTALL_FAILED;
...
public class DefaultInstallListener implements OnInstallListener {
    ...
    public boolean onInstallApk(@NonNull Context context, @NonNull File apkFile, @NonNull DownloadEntity downloadEntity) {
        if (checkApkFile(downloadEntity, apkFile)) {
            ...
            _AppSpace.onUpdateError(INSTALL_FAILED, "Apk file verify failed, please check whether the MD5 value you set is correct！");
            return false;
        }
    }
    ...
    protected boolean installApkFile(Context context, File apkFile) {
        try {
            ...
            _AppSpace.onUpdateError(INSTALL_FAILED, "An error occurred while install apk:" + e.getMessage());
        }
        return false;
    }
