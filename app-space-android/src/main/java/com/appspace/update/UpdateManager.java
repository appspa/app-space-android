...
import com.appspace.update.proxy.impl.DefaultUpdatePrompter;
...
public class UpdateManager implements IUpdateProxy {
    ...
    private void doCheck() {
        if (mIsWifiOnly) {
            if (UpdateUtils.checkWifi()) {
                ...
                onAfterCheck();
                _AppSpace.onUpdateError(UpdateError.ERROR.CHECK_NO_WIFI);
            }
        } else {
            if (UpdateUtils.checkNetwork()) {
                ...
                onAfterCheck();
                _AppSpace.onUpdateError(UpdateError.ERROR.CHECK_NO_NETWORK);
            }
        }
    }
            ...
            if (mUpdateProxy != null) {
                ...
                if (mIUpdatePrompter instanceof DefaultUpdatePrompter) {
                    ...
                    if (context instanceof FragmentActivity && ((FragmentActivity) context).isFinishing()) {
                        _AppSpace.onUpdateError(UpdateError.ERROR.PROMPT_ACTIVITY_DESTROY);
                    } else {
    ...
    public static class Builder {
        ...
        Builder(@NonNull Context context) {
            ...
            params = new TreeMap<>();
            if (_AppSpace.getParams() != null) {
                params.putAll(_AppSpace.getParams());
            }

            promptEntity = new PromptEntity();

            updateHttpService = _AppSpace.getIUpdateHttpService();

            updateChecker = _AppSpace.getIUpdateChecker();
            updateParser = _AppSpace.getIUpdateParser();
            updatePrompter = _AppSpace.getIUpdatePrompter();
            updateDownLoader = _AppSpace.getIUpdateDownLoader();

            isGet = _AppSpace.isGet();
            isWifiOnly = _AppSpace.isWifiOnly();
            isAutoMode = _AppSpace.isAutoMode();
            apkCacheDir = _AppSpace.getApkCacheDir();
        }
        ...
        public Builder promptTopDrawable(Drawable topDrawable) {
            if (topDrawable != null) {
                String tag = _AppSpace.saveTopDrawable(topDrawable);
        ...
        public Builder promptTopBitmap(Bitmap topBitmap) {
            if (topBitmap != null) {
                String tag = _AppSpace.saveTopDrawable(new BitmapDrawable(context.getResources(), topBitmap));
                promptEntity.setTopDrawableTag(tag);
            }
            return this;
        }
